...
CoordCache::CoordCache(std::shared_ptr<AtomTyper> t,
                       const ExampleProviderSettings &settings,
                       const std::string &mc)
    : typer(t), data_root(settings.data_root), molcache(mc),
      use_cache(settings.cache_structs), addh(settings.add_hydrogens),
      make_vector_types(settings.make_vector_types) {
  if (molcache.length() > 0) {
    ...
    if (!mcache)
      throw invalid_argument("Could not open file: " + molcache);
    int version = 0;
    ...
    if (!cache_map.is_open())
      throw logic_error("Could not memory map " + molcache);
  ...
  if (offsets.count(fname)) {
    ...
    if (data_root.length()) {
      boost::filesystem::path p =
          boost::filesystem::path(data_root) / boost::filesystem::path(fname);
      fullname = p.string();
    }
    ...
    if (use_cache) { // save coord
      memcache[fname] =
          coord.clone(); // save a copy in case the returned set is modified
    }
  }
}

void CoordCache::save_mem_cache(std::ostream &out) const {
  boost::archive::binary_oarchive oarch(out);

  // must convert to std::string as storing pointers makes no sense
  std::unordered_map<std::string, CoordinateSet> tmp;
  tmp.insert(memcache.begin(), memcache.end());
  oarch << tmp;
}


void CoordCache::load_mem_cache(std::istream &in) {
  boost::archive::binary_iarchive iarch(in);
  std::unordered_map<std::string, CoordinateSet> tmp; 
  iarch >> tmp;
  for (const auto& kv : tmp) {
    const char *fname = string_cache.get(kv.first);
    memcache[fname] = kv.second;
  }
}
